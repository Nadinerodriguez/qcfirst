...
Course.getAll = result => {
  pool.getConnection((err, connection) => {
    ...
    connection.query("SELECT * FROM courses ORDER BY course_name ASC, course_start_time ASC", (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
...
Course.updateById = (course_id, course, result) => {
  pool.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(
      "UPDATE courses SET faculty_id = ?, dept_id = ?, course_name = ?, course_credits = ?, course_section = ?, course_season = ?, course_year = ?, course_room = ?, course_capacity = ?, course_start_time = ?, course_end_time = ?, course_desc = ?, enroll_deadline = ?, number_enrolled = ? WHERE course_id = ?",
      [course.faculty_id, course.dept_id, course.course_name, 
        ...
        course.enroll_deadline, course.number_enrolled, course_id],
      (err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
